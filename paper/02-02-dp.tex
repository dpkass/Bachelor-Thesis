...
\begin{theorem}[Time and Space Complexity of the DP Algorithm] \label{thm:dp-time-space-complexity}
...
\begin{definition}[State With Predecessor] \label{def:state-with-predecessor}
...
\begin{proposition}[Number of States with $l$ Predecessors (No Overlap)] \label{prop:sp}
...
\begin{corollary}[Exact State Space Size \& Time Complexity without Overlapping Successors] \label{cor:ss-without-overlap}
...
\begin{proposition}[Number of States with $l$ Predecessors] \label{prop:sp'}
    The number of states that have $l$ predecessors for given $n$ and $m$ is described by the following function:
    \[
...
\begin{lemma}[State Space Size with Symmetry Breaking] \label{lemma:symmetry-breaking}
...
\begin{lemma}[Space Complexity with Layer-by-Layer Processing] \label{lemma:space-layer-by-layer}
